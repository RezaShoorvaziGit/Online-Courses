...
<div>
    <form action="{{route('updateexam')}}" method="POST">
        ...
        <div class="row justify-center m-2">
            <div class="col-2 ">
                <input type="submit" class="btn btn-warning" value="بروزرسانی" />
            </div>
            <div class="col-5  ">
                <div>
                    <input type="text" class="form-control text-center" name="title" value="{{ $exam->title}}" >
        ...
        <div class="row justify-center m-2">
            <div class="form-group col-8">
                <textarea class="form-control  text-center" name="description" id="description"
                    rows="3" placeholder="توضیحات آزمون" >
                    {{ $exam->title}}
                </textarea>
            </div>
        </div>
        ...
        <div class="row justify-center m-2">
            <div class="col-4">
               <div class="input-group ">
                ...
                <input type="date" name="date_of_holding" id="date_of_holding" value="{{ $exam->date_of_holding}}" >
               </div>
            </div>
            <div class="col-3">
                <div class="input-group ">
                    <input type="number" class="form-control" min="1" 
                    max="1000" name="time" placeholder="زمان آزمون"  value="{{ $exam->time}}">
        ...
        <div class="row m-3 justify-center">
            <div class="col-5">
                <select name="course_id"
                    ...
                    @endphp
                    <option value="{{ $course->id }}" {{ $selected }}>.&ensp;&ensp;{{ $course->title ." " . $course->title}} </option>
                    @endforeach
                </select>
            </div>
        </div>
